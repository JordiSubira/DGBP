...
xtr_recv_map_request(lisp_xtr_t *xtr, lbuf_t *buf, void *ecm_hdr, uconn_t *int_uc, uconn_t *ext_uc)
{
	...
	//Checking ACL on HLFabric
	if (checkPolicy(lisp_addr_to_char(deid), lisp_addr_to_char(seid)) != 0 ){
		OOR_LOG(LDBG_1,"Error trying to find policy dEID = %s and sEID = %s",
			lisp_addr_to_char(deid), lisp_addr_to_char(seid));
		goto err;
	}else {
		OOR_LOG(LDBG_1,"Succesfuly finding policy dEID = %s and sEID = %s",
			lisp_addr_to_char(deid), lisp_addr_to_char(seid));
	}
